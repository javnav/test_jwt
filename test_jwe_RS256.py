...
print "\t    |"
print "\t< Sign token (claims, private external key) >"
print "\t    |"
try:
    jwk = {'k': external_key.exportKey('PEM')}
    jws = jose.sign(claims, jwk, alg='RS256')
...
print "\t    |"
try:
    jwk = {'k': external_key.publickey().exportKey('PEM')}
    signed_jws = jose.verify(jose.deserialize_compact(jwt), jwk, 'RS256') 
    header = signed_jws[0]
    claims = signed_jws[1]
    print "\t   OK. Secret received from " + str(claims['from']) + " with timestamp " + str(claims['timestamp'])
    print "\t    |"
except:
